...
public class ChatService : IChatService
{
    ...
    public async Task<IReadOnlyList<ChatMessageDto>> GetHistoryAsync(Guid sessionId, CancellationToken cancellationToken = default)
    {
    ...
    private HttpRequestMessage BuildGeminiRequest(IReadOnlyList<ChatMessageDto> priorMessages, string latestMessage)
    {
        ...
        var prompt = string.IsNullOrWhiteSpace(historyText)
            ? latestMessage
            : $"{historyText}\nuser: {latestMessage}";
    ...
    private static string ParseAiResponse(string responseBody, bool isGemini)
    {
        ...
        return isGemini
            ? doc.RootElement
                ...
                .GetProperty("parts")[0]
                .GetProperty("text")
                .GetString() ?? "The AI returned an empty response."
            : doc.RootElement
